...
####You need following in order complete setup
1. Java 17
2. Maven
...
####
Once Build is success you need to start following two services
1. Kafka instance with below command
   
$docker-compose up -d

2. Trade Instruction capture service with below command.
...
2. For Postman please import collection from below folder
   
..\instructions-capture-service\src\test\resources

For Testing API , You can create single Trade Instruction with : http://localhost:8080/api/trades

Or Upload file with : http://localhost:8080/api/trades/upload

Once Trade instruction is created , look for logs to find id and use the ID for Get API.
--example log below
